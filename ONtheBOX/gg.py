    ...
    def play(self):
        ...
        while self.gameloop==True:
            ...
            if not game_over:
                ...
                keys = pygame.key.get_pressed()  # ← move this UP before the pray check

                if distance(tom, cat) < 100:
                    pop("Do you want to pray? press y for yes, press n for no")
                    if keys[pygame.K_y]:
                        tom.pray()        # ← load the frames
                        tom.state = "pray"  # ← set the state                # \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
                        cat.confused =True
                        cat.confused_timer = FPS * 3 
